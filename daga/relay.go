...
func (p *RelayProtocol) relaySetup() error {
	...
	rosterBytes, err := MarshalPointsMap(p.ClientPublicKeys)
	if err != nil {
	...
	for _, trustee := range p.Trustees {
		trusteeMsg, err := ReadMessage(trustee.Conn)
		if err != nil {
...
func (p *RelayProtocol) AuthenticateClient(clientConn net.Conn) (NodeRepresentation, error) {
	...
	pkBytes, err := MarshalPointsMap(p.TrusteePublicKeys)
	if err != nil {
		return NodeRepresentation{}, errors.New("Cannot marshal server public keys. " + err.Error())
	}
	pkSize := len(pkBytes)
	...
	if err := writeMessage(clientConn, welcomeMsg); err != nil {
		return NodeRepresentation{}, errors.New("Cannot write to the relay. " + err.Error())
	}

	// Wait until the client's authentication is finished
	//clientMsg, err := ReadMessage(clientConn)
	//if err != nil {
	//	return NodeRepresentation{}, errors.New("Client disconnected. " + err.Error())
	//}

	// Check validity of client's linkage tag

	return NodeRepresentation{}, nil
}
