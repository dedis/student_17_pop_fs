...
func ClientAuthentication(relayConn net.Conn, clientId int, privateKey abstract.Scalar) error {
	...
	welcomeMsg, err := ReadMessage(relayConn)
	if err != nil {
	...
	serverPublicKeys, err := UnmarshalPointsMap(CryptoSuite, welcomeMsg[3+addrSize:3+addrSize+pkSize])
	if err != nil {
	...
	commitBytes, err := ReadMessage(relayConn)
	if err != nil {
		return errors.New("Trustee disconnected. " + err.Error())
	}
	trusteeCommits, err := UnmarshalPointsMap(CryptoSuite, commitBytes)
	if err != nil {
		return errors.New("Cannot unmarshal trustee commitments. " + err.Error())
	}

	// Calculate my per-round generator h_i
	h, err := computeClientGroupGenerator(CryptoSuite, clientId, trusteeCommits)
	if err != nil {
		return err
	}
	...
	base := CryptoSuite.Point().Base()
	z := CryptoSuite.Scalar().Pick(rand) // Ephemeral private key
	//Z := CryptoSuite.Point().Mul(base, z) // Ephemeral public key

	// Compute the initial linkage tag and client's commitments (one commitment for each trustee)
	sProduct := CryptoSuite.Scalar().One()
	S := make(map[int]abstract.Point, len(serverPublicKeys)) // Client's commitments

	for j, _ := range serverPublicKeys {

		exp := CryptoSuite.Point().Mul(serverPublicKeys[j], z)
		s := hashPoint(CryptoSuite, exp)               // s_j = H(Y_j^z_i), Y_j is trustee j public key
		S[j] = CryptoSuite.Point().Mul(base, sProduct) // Client commitment: S_j = g^{s_1 * ... * s_j}
		sProduct = CryptoSuite.Scalar().Mul(sProduct, s)
	}
	initialTag := CryptoSuite.Point().Mul(h, sProduct) // T_0 = h_i^{s_1 * ... * s_m}
	...
	finalTagBytes, err := ReadMessage(trusteeConn)
	if err != nil {
		return errors.New("Trustee disconnected. " + err.Error())
	}
	finalTag := CryptoSuite.Point()
